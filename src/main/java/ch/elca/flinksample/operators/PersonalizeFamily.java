...
public class PersonalizeFamily extends RichFlatMapFunction<KafkaRecord<FamilyChange>, PersonalFamily> {

    private transient ValueState<FamilyChange> previousFamilyState;
    @Override
    public void flatMap(KafkaRecord<FamilyChange> family, Collector<PersonalFamily> collector) throws Exception {
        ...
        previousFamilyMembers.forEach(personRef ->
                collector.collect(new PersonalFamily(personRef, family.key(), family.offset(), null))
        );
        // create current personal families
        family.value().familyMemberRefs().forEach(personRef ->
                collector.collect(new PersonalFamily(personRef, family.key(), family.offset(), family.value().familyMemberRefs()))
        );
